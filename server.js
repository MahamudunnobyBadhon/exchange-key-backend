...
const feIo = new Server(FRONTEND_PORT, {
...
const beIo = new Server(PEER_PORT, { transports: ["websocket"] });
...
function setupPeerListeners() {
  ...
  peerSocket.on("plaintext_msg", (message) => {
    if (feSocket)
      feSocket.emit("new-message", { sender: "Peer", text: message });
  });
  peerSocket.on("secure_msg", (message) => {
    const decrypted = crypto.decryptMessage(message);
    if (feSocket)
      feSocket.emit("new-message", {
        sender: "Peer (Encrypted)",
        text: decrypted,
      });
  });
